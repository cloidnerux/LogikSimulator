...
NANDFunc::NANDFunc(shared_ptr<Node> a, shared_ptr<Node> b, shared_ptr<Node> out, int delay)
{
	callCount = 0;
	inputs.push_back(a);
...
void NANDFunc::Update(int cycleTime)
{
	bool state = true;
	...
	if(outputs.size() != 1)
		throw "More or less than 1 output for NAND operation!";
...
char * NANDFunc::GetType()
{
	return "NAND";
}

char * NANDFunc::GetState()
{
